...
- **修复候选链降级 bug（主因）**：开源改造 C3 后 `discoverFavicon()` 无条件返回未验证的 DDG URL，后台批量写库后 Favicon 组件将其视为可靠来源，候选链从 6 层缩成 2 层，DDG 未收录（404）域名永远字母徽章。新增 `isServiceFavicon()` 使服务 URL 不再短路候选链（存量书签渲染时自愈，无需迁移）；`tickFaviconDiscovery` 只写 `FAVICON_MAP` 命中项（零网络请求），历史遗留服务 URL 书签纳入修复
- **子域名根域兜底**：新增 `rootDomain()`（简化根域提取，内置常见双段后缀表），服务兜底段变为 DDG(域名) → DDG(根域) → Google s2，覆盖 DDG 只收录根域的情况
- 新增 `tests/favicon.test.ts` 7 条纯函数单测（总 44 条）
- 已知残余限制：图标在非标准路径且 DDG 未收录的站点仍字母徽章（彻底解决需 host 权限抓 HTML 或 `favicon` 权限读本地缓存，均按用户决策排除：不加权限、不干扰用户）
- 验证：build / lint 0 / 44 条单测 / smoke 全绿截图核对；PR #6 → CI 绿 → squash merge
